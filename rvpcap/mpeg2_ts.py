...
class MPEG2TSModel:
    ...
    def __print_pids(self):
        for k in self.cc_by_pid.keys():
            print(k)
        print("---")

    @classmethod
    def parse(cls, mpeg2_ts):
        ts_bytearray = mpeg2_ts
        r = MPEG2TS(
            ...
            if r.pid != 0: # PID0はスキップ
                if r.pid in MPEG2TSModel.cc_by_pid:
                    ...
                    if not(sub_cc == 1 or sub_cc == -15 or sub_cc == 0):
                        print(f"cc error,{dst_address},{str(datetime.datetime.fromtimestamp(timestamp))},{r.pid},{prev_cc},{current_cc}")
